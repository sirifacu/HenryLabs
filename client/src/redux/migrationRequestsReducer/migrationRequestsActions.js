...
export const getRequests = status => (dispatch, getState) => {
    return axios.get(`migrations/listAll?status=${status}`, { headers: {'Authorization': 'Bearer ' + getState().userLoggedIn.token }})
...
export const changeStatus = (requests, status, cohortId) => (dispatch, getState) => {
    const promises = requests.length ? requests.map( request => {
        return new Promise((resolve, reject) => {
            resolve (
                axios.put(`migrations/changeStatus/${request.id}`, { status }, { headers: {'Authorization': 'Bearer ' + getState().userLoggedIn.token }})
                .then(res => {
    ...
    .then(() => axios.get('migrations/listAll?status=pending', { headers: {'Authorization': 'Bearer ' + getState().userLoggedIn.token }})
